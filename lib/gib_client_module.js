...
class GIBClient {
  ...
  parseRobotBid(xmlData) {
    ...
    // PRIMARY: GIB returns actual bid in <r> element's bid attribute (purkka.js:n logiikka)
    ...
    if (bidMatch && bidMatch[1]) {
      ...
      if (validBidPattern.test(gibBid)) {
        console.log(`✅ GIB bid parsed: ${gibBid}`);
        return {
          type: 'bid',
          bid: gibBid,
    ...
    // FALLBACK: Try old c="..." parsing as backup (purkka.js:n logiikka)
    const fallbackMatch = xmlData.match(/c="([^"]+)"/);
    if (fallbackMatch && fallbackMatch[1] && fallbackMatch[1] !== '?' && fallbackMatch[1] !== 'n') {
      ...
      const validBidPattern = /^([1-7][CDHSN]|P|X|XX)$/;
      if (validBidPattern.test(gibBid)) {
        console.log(`✅ GIB bid parsed (fallback): ${gibBid}`);
        return {
          type: 'bid',
          bid: gibBid,
          explanation: this.extractExplanation(xmlData)
    ...
    // ALTERNATIVE: Try alternative parsing (purkka.js:n logiikka)
    ...
    // Jos mikään ei toimi, heitä virhe
    console.log(`❌ Could not parse any bid from GIB response`);
    throw new Error('Could not parse bid from GIB response');
  }
